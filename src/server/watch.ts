...
interface WatchOptions {
  ...
  recursive: boolean;
  /** Set of full paths to watch. For directory mode, used to filter event paths. */
 ...
 *
 * For directory mode: watches recursively, filters to .md files within
 * `knownPaths`. For single-file mode: watches the path directly and
 * re-establishes the watcher on close (macOS atomic-save workaround).
 */
  ...
  const start = (): void => {
    ...
    if (opts.recursive) {
      watcher = watch(target, { recursive: true }, (_eventType, filename) => {
        ...
        if (opts.knownPaths) {
          const fullPath = resolve(target, filename);
          if (!opts.knownPaths.has(fullPath)) return;
        }
        fire();
      });
    } else {
